...
fn serve_html(nav_html: &str, langue: &str) -> String {
    let html = include_str!("../../static/fchier/fchier.html").replace("__NAV_HTML__", nav_html);
    let html = i18n::appliquer_traductions(&html, langue, &[
        ("{{T_TITRE_ONGLET}}", Cle::FchTitreOnglet),
        ("{{T_NOUVEAU}}", Cle::FchNouveau),
        ("{{T_NOUVEAU_DOSSIER}}", Cle::FchNouveauDossier),
        ("{{T_NOUVELLE_PAGE_WEB}}", Cle::FchNouvellePageWeb),
        ("{{T_IMPORTER_FICHIERS}}", Cle::FchImporterFichiers),
        ("{{T_IMPORTER_DOSSIER}}", Cle::FchImporterDossier),
        ("{{T_MES_FICHIERS}}", Cle::FchMesFichiers),
        ("{{T_PARTAGE_AVEC_MOI}}", Cle::FchPartageAvecMoi),
        ("{{T_ACCUEIL}}", Cle::FchAccueil),
        ("{{T_TOUS_TYPES}}", Cle::FchTousTypes),
        ("{{T_DOSSIERS}}", Cle::FchDossiers),
        ("{{T_PAGES_WEB}}", Cle::FchPagesWeb),
        ("{{T_FICHIERS}}", Cle::FchFichiers),
        ("{{T_IMAGES}}", Cle::FchImages),
        ("{{T_VIDEOS}}", Cle::FchVideos),
        ("{{T_DOCUMENTS}}", Cle::FchDocuments),
        ("{{T_CODE}}", Cle::FchCode),
        ("{{T_RECHERCHE_PLACEHOLDER}}", Cle::FchRecherchePlaceholder),
        ("{{T_GRILLE}}", Cle::FchGrille),
        ("{{T_LISTE}}", Cle::FchListe),
        ("{{T_COL_NOM}}", Cle::FchColNom),
        ("{{T_COL_TYPE}}", Cle::FchColType),
        ("{{T_COL_DATE}}", Cle::FchColDate),
        ("{{T_OUVRIR}}", Cle::FchOuvrir),
        ("{{T_TELECHARGER}}", Cle::FchTelecharger),
        ("{{T_VOIR_EDITER}}", Cle::FchVoirEditer),
        ("{{T_EDITER_OFFICE}}", Cle::FchEditerOffice),
        ("{{T_RENOMMER}}", Cle::FchRenommer),
        ("{{T_VISIBILITE}}", Cle::FchVisibilite),
        ("{{T_DEPLACER}}", Cle::FchDeplacer),
        ("{{T_PARTAGER}}", Cle::FchPartager),
        ("{{T_ENVOYER_P2P}}", Cle::FchEnvoyerP2p),
        ("{{T_SUPPRIMER}}", Cle::FchSupprimer),
        ("{{T_ANNULER}}", Cle::FchAnnuler),
        ("{{T_CREER}}", Cle::FchCreer),
        ("{{T_CONFIRMER}}", Cle::FchConfirmer),
        ("{{T_FERMER}}", Cle::FchFermer),
        ("{{T_ENREGISTRER_BTN}}", Cle::FchEnregistrerBtn),
        ("{{T_APPLIQUER}}", Cle::FchAppliquer),
        ("{{T_ENVOYER}}", Cle::FchEnvoyer),
        ("{{T_SELECTIONNER_FICHIERS}}", Cle::FchSelectionnerFichiers),
        ("{{T_CLIC_OU_GLISSEZ}}", Cle::FchClicOuGlissez),
        ("{{T_PRIVE}}", Cle::FchPrive),
        ("{{T_PUBLIC}}", Cle::FchPublic),
        ("{{T_PREPARATION}}", Cle::FchPreparation),
        ("{{T_UPLOADER}}", Cle::FchUploader),
        ("{{T_NOM_DOSSIER}}", Cle::FchNomDossier),
        ("{{T_PLACEHOLDER_MON_DOSSIER}}", Cle::FchPlaceholderMonDossier),
        ("{{T_NOUVEAU_NOM}}", Cle::FchNouveauNom),
        ("{{T_CHARGEMENT}}", Cle::FchChargement),
        ("{{T_ENVOYER_VIA_P2P}}", Cle::FchEnvoyerViaP2p),
        ("{{T_ID_DESTINATAIRE}}", Cle::FchIdDestinataire),
        ("{{T_DEPLACER_VERS}}", Cle::FchDeplacerVers),
        ("{{T_DOSSIER_DESTINATION}}", Cle::FchDossierDestination),
        ("{{T_RACINE}}", Cle::FchRacine),
        ("{{T_CONFIRMER_SUPPRESSION}}", Cle::FchConfirmerSuppression),
        ("{{T_IDS_UTILISATEURS}}", Cle::FchIdsUtilisateurs),
        ("{{T_LAISSER_VIDE_POUR_REVOQUER}}", Cle::FchLaisserVidePourRevoquer),
        ("{{T_RETOUR}}", Cle::FchRetour),
        ("{{T_ENREGISTRER_ET_FERMER}}", Cle::FchEnregistrerEtFermer),
        ("{{T_CHARGEMENT_DOCUMENT}}", Cle::FchChargementDocument),
    ]);
    let i18n_js = i18n::objet_js(langue, &[
        ("ERREUR", Cle::FchErreur),
        ("PAGE_CREE", Cle::FchPageCree),
        ("ERREUR_PREFIX", Cle::FchErreurPrefix),
        ("IMPORTER_FICHIERS", Cle::FchImporterFichiers),
        ("SELECTIONNER_FICHIERS", Cle::FchSelectionnerFichiers),
        ("CLIC_OU_GLISSEZ", Cle::FchClicOuGlissez),
        ("IMPORTER_DOSSIER", Cle::FchImporterDossier),
        ("SELECTIONNER_DOSSIER", Cle::FchSelectionnerDossier),
        ("CLIQUEZ_CHOISIR_DOSSIER", Cle::FchCliquezChoisirDossier),
        ("FICHIER_DANS_DOSSIER_PLUR", Cle::FchFichierDansDossierPlur),
        ("FICHIER_DANS_DOSSIER_SING", Cle::FchFichierDansDossierSing),
        ("UN_FICHIER_SELECTIONNE", Cle::FchUnFichierSelectionne),
        ("N_FICHIERS_SELECTIONNES", Cle::FchNFichiersSelectionnes),
        ("RETIRER", Cle::FchRetirer),
        ("UNITE_OCTET", Cle::FchUniteOctet),
        ("UNITE_KO", Cle::FchUniteKo),
        ("UNITE_MO", Cle::FchUniteMo),
        ("UNITE_GO", Cle::FchUniteGo),
        ("UPLOAD_EN_COURS", Cle::FchUploadEnCours),
        ("CHIFFREMENT_NOM", Cle::FchChiffrementNom),
        ("ECHEC", Cle::FchEchec),
        ("TERMINE_AVEC_ERREURS", Cle::FchTermineAvecErreurs),
        ("TERMINE", Cle::FchTermine),
        ("UPLOADES_ERREURS", Cle::FchUploadesErreurs),
        ("N_FICHIERS_UPLOADES", Cle::FchNFichiersUploades),
        ("ANALYSE_ARBORESCENCE", Cle::FchAnalyseArborescence),
        ("CREATION_DOSSIERS_PROGRESS", Cle::FchCreationDossiersProgress),
        ("IMPOSSIBLE_CREER_DOSSIER", Cle::FchImpossibleCreerDossier),
        ("ERREUR_DOSSIER_NOM", Cle::FchErreurDossierNom),
        ("FICHIERS_PROGRESS", Cle::FchFichiersProgress),
        ("FICHIERS_UPLOADES_ERREURS", Cle::FchFichiersUploadesErreurs),
        ("DOSSIER_IMPORTE", Cle::FchDossierImporte),
        ("UPLOADER", Cle::FchUploader),
        ("MES_FICHIERS", Cle::FchMesFichiers),
        ("AUCUN_FICHIER", Cle::FchAucunFichier),
        ("CREEZ_PREMIER_DOCUMENT", Cle::FchCreezPremierDocument),
        ("DOSSIER_LABEL", Cle::FchDossierLabel),
        ("PAGE_WEB_LABEL", Cle::FchPageWebLabel),
        ("DEPLACE", Cle::FchDeplace),
        ("SUPPRIME", Cle::FchSupprime),
        ("ACCUEIL", Cle::FchAccueil),
        ("RACINE", Cle::FchRacine),
        ("TELECHARGEMENT_DE", Cle::FchTelechargementDe),
        ("SESSION_EXPIREE_TELECHARGER", Cle::FchSessionExpireeTelecharger),
        ("SESSION_EXPIREE_OUVRIR", Cle::FchSessionExpireeOuvrir),
        ("NOM_TELECHARGE", Cle::FchNomTelecharge),
        ("ERREUR_TELECHARGEMENT_PREFIX", Cle::FchErreurTelechargementPrefix),
        ("CONFIRM_OUVRIR_ONLYOFFICE", Cle::FchConfirmOuvrirOnlyOffice),
        ("EDITER_AVEC_ONLYOFFICE", Cle::FchEditerAvecOnlyOffice),
        ("PREPARATION_DOCUMENT", Cle::FchPreparationDocument),
        ("SESSION_EXPIREE_RECONNECTEZ", Cle::FchSessionExpireeReconnectez),
        ("PRET", Cle::FchPret),
        ("ERREUR_ONLYOFFICE", Cle::FchErreurOnlyOffice),
        ("IMPOSSIBLE_CHARGER_ONLYOFFICE", Cle::FchImpossibleChargerOnlyOffice),
        ("CONFIRM_FERMER_SANS_ENREGISTRER", Cle::FchConfirmFermerSansEnregistrer),
        ("FERMER_SANS_ENREGISTRER", Cle::FchFermerSansEnregistrer),
        ("SAUVEGARDE_EN_COURS_OO", Cle::FchSauvegardeEnCoursOO),
        ("RECUPERATION_CONTENU", Cle::FchRecuperationContenu),
        ("ECHEC_RECUPERATION", Cle::FchEchecRecuperation),
        ("CHIFFREMENT_EN_COURS", Cle::FchChiffrementEnCours),
        ("ENREGISTREMENT_EN_COURS", Cle::FchEnregistrementEnCours),
        ("ECHEC_ENREGISTREMENT_PT", Cle::FchEchecEnregistrementPt),
        ("DOCUMENT_ENREGISTRE", Cle::FchDocumentEnregistre),
        ("VOIR_EDITER", Cle::FchVoirEditer),
        ("CHARGEMENT", Cle::FchChargement),
        ("ERREUR_OUVERTURE_PREFIX", Cle::FchErreurOuverturePrefix),
        ("ENREGISTRE_PT", Cle::FchEnregistrePt),
        ("FICHIER_ENREGISTRE", Cle::FchFichierEnregistre),
        ("ERREUR_ENREGISTREMENT_PREFIX", Cle::FchErreurEnregistrementPrefix),
        ("DOSSIER_CREE", Cle::FchDossierCree),
        ("RENOMME", Cle::FchRenomme),
        ("PARTAGE_MIS_A_JOUR", Cle::FchPartageMisAJour),
        ("VISIBILITE_MISE_A_JOUR", Cle::FchVisibiliteMiseAJour),
        ("ID_DESTINATAIRE_REQUIS", Cle::FchIdDestinataireRequis),
        ("ENVOI_ECHOUE", Cle::FchEnvoiEchoue),
        ("ENVOI_P2P_LANCE", Cle::FchEnvoiP2pLance),
        ("ERREUR_ENVOI_P2P_PREFIX", Cle::FchErreurEnvoiP2pPrefix),
        ("MODIFIER", Cle::FchModifier),
        ("OUVRIR", Cle::FchOuvrir),
        ("CONFIRMER_SUPPRESSION_MSG", Cle::FchConfirmerSuppressionMsg),
        ("CET_ELEMENT", Cle::FchCetElement),
        ("CONFIRMER", Cle::FchConfirmer),
        ("AUJOURDHUI", Cle::FchAujourdhui),
        ("HIER", Cle::FchHier),
        ("N_JOURS", Cle::FchNJours),
        ("LOCALE_DATE", Cle::FchLocaleDate),
    ]);
    html.replacen("{{I18N_JS}}", &i18n_js, 1)
}
// ══════════════════════════════════════════════════════════════════
// api_prefs
