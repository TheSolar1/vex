...
$connecte ="1";
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST" && $connecte == "1") {
          ...
          $text = $_POST['content'];
          $id = $_GET['idpage'] ?? '';
          $stmt = $conn->prepare("SELECT * FROM sitec WHERE idpage=?");
          $stmt->bind_param("i", $id);
          $stmt->execute();
          $resultpage = $stmt->get_result();
           if ($resultpage->num_rows == 1) {
           $row = $resultpage->fetch_assoc();
           
           $page_url = $row['urlpage'];
        }
           
          // Ouvrir le fichier "bonjour" en mode ecriture
          $handle = fopen($page_url .".php", "w");
     ...
     ".$script."".$text."
       ...
       $text = $textb;
         ...
         // Definir le chemin vers le fichier dans le dossier "pages"
            $chemin_fichier = '/var/www/html/sitec/pages/' .   $page_url. ".php"; // Utilisez './pages/mon_fichier.txt' si vous etes dans un dossier different
         ...
         if ($handle) {
             ...
             echo "<p>Page modifier avec succes</a></p>";
         ...
         if (isset($_GET['idpage'])){
            $idpagess = $_GET['idpage'];
            $stmt = $conn->prepare("SELECT user_id FROM sitec WHERE idpage=?");
            $stmt->bind_param("i", $id);
            $stmt->execute();
            $resultpage = $stmt->get_result();
             if ($resultpage->num_rows == 1) {
         ...
         $scalese = "1";
            if (!isset($_POST['scales'])) {
              ...
              $stmt_insert = $conn->prepare("UPDATE nompage = ?,`user_id` = ?,porb = ? WHERE  	idpage = ?");
 
              $stmt_insert->bind_param("siii",$page_name ,$user_id, $scalese,$idpagess);
        ...
        $id = $_GET['idpage'] ?? '';
                    ...
                    $resultpage = $stmt->get_result();
                     ...
                     $crateur = $row['user_id'];
                     $pageprob = $row['prob'];
                     $pageurl = "/var/www/html/sitec/pages/". $page_url .".php";

                        if ( $crateur !== $user_id){exit;}
                     if (file_exists($pageurl)) {
                        $content = file_get_contents($pageurl);
                        if ($content !== false) {
                            // Supprimer le head du contenu HTML
                        
                            preg_match('/<title>\s*(.*?)\s*<\/title>/', $content, $matches);

                            // Afficher la valeur trouvée entre les balises <title>
                           
                            $pageTitle = isset($matches[1]) ? $matches[1] : " ";
                       
                            $content = preg_replace('/<head>.*?<\/head>/s', '', $content);
                            // Échapper les caractères HTML pour afficher le code source
                    ...
                    if (file_exists($pageurl)) {
                        ...
                        if ($content !== false) {
                            ...
                            // Supprimer le include et les balises HTML inutiles
                            $content = preg_replace('/<\?php\s+include\s+[\'"][^\'"]+[\'"]\s*;\s*\?>/i', '', $content);
                            ...
                            $patterns_if = [
                                'ip' => [
                                    'label' => "Si l'adresse IP est",
                                    'regex' => '/if\s*\(\s*\'([\d\.]+)\'\s*===\s*\$_SERVER\[\'REMOTE_ADDR\'\]\s*\)\s*{\s*echo\s*"(.*?)"\s*;\s*}/s'
                                ],
                                'date' => [
                                    'label' => "Si la date est",
                                    'regex' => '/if\s*\(\s*\'(.+?)\'\s*===\s*date\(\s*\'d\/m\/Y\'\s*\)\s*\)\s*{\s*echo\s*"(.*?)"\s*;\s*}/s'
                                ],
                                'username' => [
                                    'label' => "Si l'utilisateur est",
                                    'regex' => '/if\s*\(\s*\'(.+?)\'\s*===\s*\$_SESSION\[\'username\'\]\s*\)\s*{\s*echo\s*"(.*?)"\s*;\s*}/s'
                                ],
                                'browser' => [
                                    'label' => "Si le navigateur est",
                                    'regex' => '/if\s*\(\s*\'(.+?)\'\s*===\s*\$br\s*\)\s*{\s*echo\s*"(.*?)"\s*;\s*}/s'
                                ],
                                'os' => [
                                    'label' => "Si le os est",
                                    'regex' => '/if\s*\(\s*\'(.+?)\'\s*===\s*\$os\s*\)\s*{\s*echo\s*"(.*?)"\s*;\s*}/s'
                                ]
                            ];
                            ...
                            $patterns_not = [
                                'ip' => [
                                    'label' => "Si l'adresse IP n'est pas",
                                    'regex' => '/if\s*\(\s*\'([\d\.]+)\'\s*!==\s*\$_SERVER\[\'REMOTE_ADDR\'\]\s*\)\s*{\s*echo\s*"(.*?)"\s*;\s*}/s'
                                ],
                                'date' => [
                                    'label' => "Si la date n'est pas",
                                    'regex' => '/if\s*\(\s*\'(.+?)\'\s*!==\s*date\(\s*\'d\/m\/Y\'\s*\)\s*\)\s*{\s*echo\s*"(.*?)"\s*;\s*}/s'
                                ],
                                'username' => [
                                    'label' => "Si l'utilisateur n'est pas",
                                    'regex' => '/if\s*\(\s*\'(.+?)\'\s*!==\s*\$_SESSION\[\'username\'\]\s*\)\s*{\s*echo\s*"(.*?)"\s*;\s*}/s'
                                ],
                                'browser' => [
                                    'label' => "Si le navigateur n'est pas",
                                    'regex' => '/if\s*\(\s*\'(.+?)\'\s*!==\s*\$br\s*\)\s*{\s*echo\s*"(.*?)"\s*;\s*}/s'
                                ],
                                'os' => [
                                    'label' => "Si le os n'est pas",
                                    'regex' => '/if\s*\(\s*\'(.+?)\'\s*!==\s*\$os\s*\)\s*{\s*echo\s*"(.*?)"\s*;\s*}/s'
                                ]
                            ];
                    
                            // Remplacer les conditions
                            function replaceConditions(&$content, $patterns, $typeLabel) {
                                foreach ($patterns as $key => $data) {
                                    if (preg_match_all($data['regex'], $content, $matches, PREG_SET_ORDER)) {
                                        foreach ($matches as $match) {
                                            $value = $match[1];
                                            $texte = cleanAndFormatText($match[2]);
                    
                                            $replacement = "<p><!-- Condition $typeLabel --></p><p><strong>Condition:</strong> {$data['label']} $value</p><p>$texte</p><hr>";
                                            $content = "<hr> ".str_replace($match[0], $replacement, $content);
                            ...
                            replaceConditions($content, $patterns_if, "IF");
                            replaceConditions($content, $patterns_not, "NOT");
...
<head>
	<script>
     ...
     <link rel="icon" type="image/png" href="/vex.png">
    <title>modifierpages</title>
    <?php
    if ($connecte === "1") {
echo"<script src='https://cdn.tiny.cloud/1/oxskldvmp8n098bdug7zy52h8kb872ropm4mjsqg6i84qxm3/tinymce/7/tinymce.min.js' referrerpolicy='origin'></script>   ";
}else {
...
<form method="post" action="">
    ...
    <input type="text" id="page_name" name="page_name" maxlength="20" required value="<?php echo htmlspecialchars($pageTitle); ?>">
...
tinymce.init({
    ...
    toolbar: ' conditional_if conditional_not | undo redo | blocks fontfamily fontsize | bold italic underline strikethrough | link image media table mergetags | addcomment showcomments | spellcheckdialog a11ycheck typography | align lineheight | checklist numlist bullist indent outdent | emoticons charmap | removeformat ',
